...
public class MonitorEventHandler implements InitializingBean {
	...
	public Class<?> createBeanClass(final String className, final Map<String, Class<?>> properties) {
	...
	public void writeDataLogId(UUID id) {
		try {
			BufferedWriter bw = new BufferedWriter(new FileWriter("idLog.csv"));
			bw.write(id + "\n");
			bw.close();
		} catch (IOException e) {
